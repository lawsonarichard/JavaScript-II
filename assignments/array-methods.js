...
let allCaps = [];
runners.map((runners, index) => { allCaps[index] = runners.first_name.toUpperCase(); 
console.log(allCaps); 
});
...
ticketPriceTotal = runners.reduce((reducer, runners) => {
...
runners.forEach(runner => {
    LastNameFirstName.push(`${runner.last_name}, ${runner.first_name}`);
});
...
// Problem 3
// Mass Email for shirt sizes S
let smallShirts = [];
runners.filter(runners => 
    { if (runners.shirt_size === 'S')
    return runners.email; 
});
